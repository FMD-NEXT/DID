    ...
    with torch.no_grad():
        ...
        for batch in tqdm(eval_iter):
            ...
            for _ in range(args.monte_carlo_timesteps):
                loss = loss_fn(model, batch)[0].mean()
                cur_loss += loss
...
def run(rank, args, port):
    def setup(rank, world_size, port):
        ...
        dist.init_process_group(
            "nccl", rank=rank, world_size=world_size, timeout=datetime.timedelta(minutes=300)
        )
...
def main():
    ...
    parser.add_argument("--work_dir", type=str, default="./logs/radd-t-dce")
    parser.add_argument("--loss_type", type=str, default="t_DCE")
