...
def get_lambada_test_dataset():
    ...
    lambada_data = _read_jsonl_to_list(file)
    dataset = Dataset.from_list(lambada_data)
    return dataset

def get_dataset(name, mode, cache_dir=None, block_size=1024, num_proc=len(os.sched_getaffinity(0)), take=-1):
    if name == "wikitext103":
        ...
        dataset = load_dataset('scientific_papers', 'arxiv',cache_dir=cache_dir)
    elif name == 'scientific_papers_pubmed':
        dataset = load_dataset('scientific_papers', 'pubmed',cache_dir=cache_dir)
    elif name == 'ag_news':
    ...
    def preprocess_and_tokenize(example):
        ...
        tokens = tokenizer(
            text, return_attention_mask=False)
...
def get_dataloaders(config, distributed=True):
    ...
    train_set = get_dataset(config.data.train, "train", cache_dir=config.data.cache_dir, block_size=config.model.length, take=config.data.take)
