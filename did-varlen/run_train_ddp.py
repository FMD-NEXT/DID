...
def _run(local_rank, gloab_rank, world_size, cfg):
    ...
    if gloab_rank == 0:
        ...
        wandb.init(dir=os.path.abspath(work_dir), project='sedd', config=OmegaConf.to_container(cfg, resolve=True),
                   name=cfg.wandb_name, job_type='train', mode='offline')
    ...
    # build score model 
    if 'radd' in cfg.model.name.lower(): 
        model = RADD(cfg).to(device)
        token_dim = cfg.tokens + 1
    elif 'did' in cfg.model.name.lower(): 
        model = DID(cfg).to(device)
        token_dim = cfg.tokens # + 1

    model = DDP(model, device_ids=[local_rank], static_graph=True, find_unused_parameters=True)

    num_parameters = sum(p.numel() for p in model.parameters())
    mprint(f"Number of parameters in the model: {num_parameters}")
    num_parameters_ne = sum(p.numel() for n,p in model.named_parameters() if 'blocks' in n)
    mprint(f"Number of non-embedding parameters in the model: {num_parameters_ne}")

    ema = ExponentialMovingAverage(
        model.parameters(), decay=cfg.training.ema)
    mprint(model)
    mprint(f"EMA: {ema}")
    ...
    EOS = tokenizer.encode(tokenizer.unk_token)[0]
    BOS = tokenizer.encode(tokenizer.unk_token)[0]
    ...
    if cfg.training.snapshot_sampling:
        ...
        sampler = DiffusionSampler(cfg.sampling.predictor,model,noise,sampling_shape,token_dim, strategy = 'direct',device = device, BOS=BOS)


    num_train_steps = cfg.training.n_iters
    mprint(f"Starting training loop at step {initial_step}.")

    def log_and_wandb(matric, step, name="train_loss", pattern='e'):
        dist.all_reduce(matric)
        matric /= world_size
        if pattern == 'e':
            mprint(f"step: %d, {name}: %.5e" % (step, matric.item()))
        elif pattern == 'f':
            mprint(f"step: %d, {name}: %.5f" % (step, matric.item()))

        if gloab_rank == 0:
            wandb.log({name: matric.item()}, step=step)
    
    nvtx.mark('before while')

    while state['step'] < num_train_steps + 1:
        step = state['step']

        nvtx.range_push(f'{step = }')
        if cfg.data.train != "text8":
            batch = next(train_iter)['input_ids'].to(device, non_blocking=True)
        else:
            batch = next(train_iter).to(device, non_blocking=True)
        
        # try:
        loss, (l1, l2, c), (grad_sparsity, grad_norm, weight_norm), (d1, d2) = train_step_fn(state, batch)
        nvtx.range_pop()
        ...
        if step != state['step']:
            if step % cfg.training.log_freq == 0:
                ...
                # loss details
                # log_and_wandb(l1, step, 'l1')
                # log_and_wandb(l2, step, 'l2')
                # log_and_wandb(c, step, 'c')

                # log_and_wandb(weight_norm, step, 'w_norm')
                # log_and_wandb(grad_norm, step, 'g_norm')
                # log_and_wandb(grad_sparsity, step, 'g_sparsity', 'f')
                # log_and_wandb(torch.tensor(d1*cfg.training.log_freq, device=local_rank), step, 'd1')
                # log_and_wandb(torch.tensor(d2, device=local_rank), step, 'd2')

            if step != 0 and step % cfg.training.snapshot_freq_for_preemption == 0 and gloab_rank == 0:
            ...
            if step % cfg.training.eval_freq == 0:
                ...
                eval_loss, _, _, _ = eval_step_fn(state, eval_batch)
                    ...
                    if cfg.eval.perplexity:
                        with torch.no_grad():
                            ...
                            if isinstance(sample, torch.Tensor):
                                ...
                                batches = len(sample) // cfg.eval.perplexity_batch_size

                            total_perplexity = 0
                            for i in range(batches):
                                s = sample[i * cfg.eval.perplexity_batch_size:(i + 1) * cfg.eval.perplexity_batch_size] # 16 sentences
...
def calculate_perplexity(batch, model, tokenizer):
    """
    计算变长batch的perplexity，与tensor版本逻辑一致
    :param batch: List[List[int]], 变长的token id序列
    :param model: 已加载的GPT2模型
    :param tokenizer: GPT2 tokenizer
    :return: float, 整个batch的平均perplexity
    """
    # 找到batch中最长的序列长度
    ...
    # 与tensor版本一致的计算方式
    logits = logits.transpose(-1, -2)  # 准备交叉熵输入 [batch, vocab, seq]
    
    # 计算每个token的交叉熵损失
    loss = F.cross_entropy(
        logits[..., :-1],  # 预测前n-1个token
        input_ids[..., 1:],  # 目标是后n-1个token
        ignore_index=tokenizer.pad_token_id,  # 忽略padding部分
        reduction='none'
    )
    # print(loss)
