...
def gumbel_softmax(categorical_probs, hard=False, eps=1e-50):
    logits = categorical_probs.clamp(min=eps).log()
...
def sample_categorical(categorical_probs, method="hard"):
    if method == "hard":
        eps = 1e-10
        gumbel_norm = eps - (torch.rand_like(categorical_probs) + eps).log()
...
def top_k_sampling(logits, k=400):
    ...
    index = sample_categorical(top_k_probs.to(torch.float64))
